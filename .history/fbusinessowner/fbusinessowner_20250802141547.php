...
<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
         ...
         $stmt_checking = $conn->prepare("SELECT fbowner_id FROM fb_owner WHERE user_id = ?");
        ...
        if ($result_checking->num_rows > 0) {
            ...
            if (!isset($gallery_json) || empty($gallery_json)) {
              $gallery_json = json_encode([]); // Always set, even if no upload
          }
      ...
      <div class="bg-white p-6 rounded-lg shadow mb-8">
        ...
        <form method="post" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6" id="business-info-form">
          ...
          <!-- Upload Business Gallery Images -->
            <div class="col-span-2">
              <!-- Display current business photo if exists -->
              <?php if (!empty($row['fb_photo'])): ?>
                <div class="mb-2">
                  <img src="<?php echo htmlspecialchars($row['fb_photo']); ?>" alt="Business Photo" class="w-32 h-32 object-cover rounded shadow">
                </div>
              <?php endif; ?>
