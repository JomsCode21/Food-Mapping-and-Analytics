...
<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
          ...
          if (!empty($_FILES['business_photo']['name'])) {
                $upload_dir = '../uploads/business_photo/';
                if (!is_dir($upload_dir)) {
                    mkdir($upload_dir, 0777, true);
                }
                
                $business_photo = $_FILES['business_photo']['name'];
                $file_tmp = $_FILES['business_photo']['tmp_name'];
                $file_ext = strtolower(pathinfo($business_photo, PATHINFO_EXTENSION));
                $file_name = uniqid() . '.' . $file_ext;
                $business_photo_path = $upload_dir . $file_name;
                
                if (move_uploaded_file($file_tmp, $business_photo_path)) {
                    $fields[] = "fb_photo = ?";
          ...
          $gallery_paths = [];
if (!empty($_FILES['business_gallery']['name'][0])) {
    $upload_dir = '../uploads/business_images/';
    if (!is_dir($upload_dir)) {
        mkdir($upload_dir, 0777, true);
    }
    
    foreach ($_FILES['business_gallery']['tmp_name'] as $key => $tmp_name) {
        $file_name = $_FILES['business_gallery']['name'][$key];
        $file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
        $new_name = uniqid() . '.' . $file_ext;
        $file_path = $upload_dir . $new_name;
        
        if (move_uploaded_file($tmp_name, $file_path)) {
            $gallery_paths[] = $file_path;
        }
    }
    
    if (!empty($gallery_paths)) {
        $fields[] = "fb_images = ?";
        $params[] = json_encode($gallery_paths);
        $types .= 's';
    }
}
          ...
          if ($result_checking->num_rows > 0) {
            ...
            $photo_path = $business_photo_path ?? null;
            $gallery_json = $business_gallery_json ?? null;
            ...
            echo json_encode(['status'=> 'error', 'message'=> 'Failed saving business profile.']);
          }
            $stmt_insert->close();
          }
          $stmt_checking->close();
