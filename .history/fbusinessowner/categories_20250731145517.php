...
$result = $conn->query("SELECT user_id, fb_name, fb_type, fb_address, fb_photo FROM fb_owner WHERE fb_type != '' AND fb_type IS NOT NULL");
...
if (isset($category_map[$cat])) {
    ...
    $stmt = $conn->prepare("SELECT fb_name, fb_address, fb_photo, user_id FROM fb_owner WHERE fb_type = ?");
...
<div id="business-list" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
  ...
  ?>
    <div class="bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
      <img 
        src="<?php echo htmlspecialchars($biz['fb_photo'] ?: '../imgsource/default.jpg'); ?>" 
        alt="<?php echo htmlspecialchars($biz['fb_name']); ?>" 
        class="w-full h-32 object-cover"
      >
      <div class="p-4">
        <h4 class="font-semibold text-lg">
          <?php echo htmlspecialchars($biz['fb_name']); ?>
          <?php if($count <= 3): ?>
            <br><span class="text-sm text-gray-500">Top <?php echo $count; ?></span>
          <?php endif; ?>
        </h4>
        <div class="flex items-center space-x-2 mt-2">
          <span class="text-yellow-500">★★★★★</span>
        ...
        <div class="flex justify-between items-center mt-3">
          <a 
            href="businessdetails.php?user_id=<?php echo urlencode($biz['user_id']); ?>" 
            class="text-primary transition-transform duration-200 hover:scale-110"
          >
            View Details
          </a>
          <button class="favorite-btn text-gray-600 hover:text-red-500">
  ...
  <?php 
    endforeach; 
  elseif(isset($category_map[$cat])): 
  ?>
    <p class="text-center text-gray-600 col-span-4">No businesses found in this category</p>
  <?php endif; ?>
</div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    // Refresh the page when coming back from cache
...
function refreshCategories() {
    ...
    if (currentCategory) {
        window.location.href = `categories.php?cat=${currentCategory}`;
    } else {
        window.location.reload();
    }
}

// Add this to your existing filterBusinesses function
function filterBusinesses() {
    const searchTerm = document.getElementById('search-bar').value.toLowerCase();
    const businessCards = document.querySelectorAll('#business-list > div');
    
    businessCards.forEach(card => {
        const businessName = card.querySelector('h4').textContent.toLowerCase();
        const businessAddress = card.querySelector('.text-gray-600').textContent.toLowerCase();
        
        if (businessName.includes(searchTerm) || businessAddress.includes(searchTerm)) {
            card.style.display = '';
        } else {
