...
if (isset($category_map[$cat])) {
    ...
    $stmt = $conn->prepare("
    SELECT fb_name, fb_address, fb_photo, user_id, fb_status
    FROM fb_owner
    WHERE fb_type = ?
    ORDER BY fbowner_id DESC
");
...
<div id="business-list" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
  ...
  <div class="p-4">
    ...
    <div class="flex items-center justify-end space-x-2 mt-2">
      <?php
        $is_open = strtolower($biz['fb_status']) === 'open';
        $circle_color = $is_open ? 'bg-green-500' : 'bg-red-500';
        $status_text = $is_open ? 'Open' : 'Closed';
      ?>
      <span class="inline-block w-3 h-3 rounded-full <?php echo $circle_color; ?>"></span>
      <span class="text-xs font-semibold <?php echo $is_open ? 'text-green-700' : 'text-red-700'; ?>">
...
function filterBusinesses() {
  ...
  if (query === '') {
    refreshCategory(); // Show all if empty
    return;
  }
  fetch('search_business.php?q=' + encodeURIComponent(query))
    .then(res => res.json())
    .then(data => {
      const container = document.getElementById('business-list');
      container.innerHTML = '';
      if (data.length === 0) {
        container.innerHTML = '<p class="text-center text-gray-600 col-span-4">No businesses found</p>';
        return;
      }
      data.forEach((biz, index) => {
        const isOpen = biz.fb_status.toLowerCase() === 'open';
        const circleColor = isOpen ? 'bg-green-500' : 'bg-red-500';
        const statusText = isOpen ? 'Open' : 'Closed';
        container.innerHTML += `
          <div class="bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
            <img src="${biz.fb_photo || '../imgsource/default.jpg'}" alt="${biz.fb_name}" class="w-full h-32 object-cover">
            <div class="p-4">
              <h4 class="font-semibold text-lg">${biz.fb_name}<br>
                <span class="text-sm text-gray-500">Top ${index + 1}</span>
              </h4>
              <div class="flex items-center space-x-2 mt-2">
                <span class="text-yellow-500">★★★★★</span>
                <span class="text-gray-600">${biz.fb_address}</span>
              </div>
              <div class="flex items-center justify-end space-x-2 mt-2">
                <span class="inline-block w-3 h-3 rounded-full ${circleColor}"></span>
                <span class="text-xs font-semibold ${isOpen ? 'text-green-700' : 'text-red-700'}">${statusText}</span>
              </div>
              <div class="flex justify-between items-center mt-3">
                <a href="businessdetails.php?user_id=${encodeURIComponent(biz.user_id)}" class="text-primary transition-transform duration-200 hover:scale-110">View Details</a>
                <button class="favorite-btn text-gray-600 hover:text-red-500">
                  <i class="ri-heart-line text-2xl"></i>
                </button>
              </div>
