...
<footer class="bg-primary text-white mt-10 px-4 py-6">
  ...
  // Calculate bounds from Libmanan coordinates
  const bounds = new google.maps.LatLngBounds();
  libmananCoords.forEach(pt => bounds.extend(pt));

  // Large rectangle covering the map (make sure it's closed)
  const outerCoords = [
    {lat: 14.0, lng: 122.6},
    {lat: 14.0, lng: 123.3},
    {lat: 13.4, lng: 123.3},
  ...
  // Initialize the map AFTER bounds is defined
  ...
  // Fit map to Libmanan bounds
  map.fitBounds(bounds);

 // Mask polygon (draw first, with a hole for Libmanan)
  const maskPolygon = new google.maps.Polygon({
    paths: [outerCoords, libmananCoords], // outer boundary, then hole
